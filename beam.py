...
class PlaybookResource(BaseModel):
    ...
    resource_type: str = Field(description="The type of Ansible resource to generate (playbooks/roles).")
    file_name: str = Field(description="The name of the ansible playbook being generated i.e remove_vim.yml")
...
class RoleResource(BaseModel):
    ...
    resource_type: str = Field(description="The type of Ansible resource to generate (playbooks/roles).")
    file_name: str = Field(description="The name of the ansible role resource being generated i.e {role_dir}/tasks/main.yml or vars/main.yml")
    task: str = Field(description="The generated Ansible resource in the tasks/ dir.")
    handlers: str = Field(description="The generated Ansible resource in the handler/ dir.")
    vars: str = Field(description="The generated Ansible resource in the vars/ dir.")
    defaults: str = Field(description="The generated Ansible resource in the defaults/ dir.")
...
def get_user_input():
    resource_type = input(f"{bcolors.OKBLUE}What kind of Ansible resource do you want to create (playbook/role)?{bcolors.ENDC} ").strip().lower()
    if resource_type not in ['playbook', 'role']:
        print(f"{bcolors.FAIL}Invalid input. Please enter 'playbook' or 'role'.{bcolors.ENDC}")
        return get_user_input()

    description = input(f"{bcolors.OKBLUE}What should the {resource_type} do? {bcolors.ENDC}").strip()
...
def load_docs():
    ...
    playbook_loader = DirectoryLoader(path=playbook_path, glob="**/*.yml", show_progress=False)
...
def main():
    ...
    new_resource = chain.invoke({"description" : description, "resource_type": resource_type})
    ...
    confirmation = input(f"{bcolors.WARNING}Do you want to save the generated Ansible resource? (yes/no): {bcolors.ENDC}")
    if confirmation.lower() == 'yes':
        save_ansible_resource(new_resource.output.resource_type, new_resource.output.file_name, new_resource.output.playbook)
    else:
